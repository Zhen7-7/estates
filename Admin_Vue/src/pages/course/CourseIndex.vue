<template>
  <div>
    ...
    <table-box ref="tableBox" :info="estate" :length="length" :url="table.url" :columns="table.columns" :key-name="table.keyName" :parse-data="table.parseData" :data="table.data" :multi-select="table.multiSelect">
        ...
        <tr slot="row"  slot-scope="props" @click="getId(props.item.robid)">
          <td class="tc" style="line-height:150px">{{props.item.robid}}</td>
          <td class="tc" style="line-height:150px">{{props.item.estatename}}</td>
          <td class="tc"><img :src="props.item.robimg" width="150" height="150"/></td>
          <td class="tc" style="line-height:150px">{{props.item.estateaddress}}</td>
          <td class="tc" style="line-height:150px">{{props.item.information}}</td>
          <td class="tc" style="line-height:150px">余{{props.item.lastnum}}户</td>
...
<script>
  ...
  export default {
    ...
    methods:{
      getData:function(){
        var that = this;
            this.$axios.post('/robhousing/getAllRobHousing  ')
