  ...
  <div class="table-wrap" :class="{scroll:scroll}">
    ...
    <div class="table-body" ref="tableBody">
      <table class="table table-bordered">
        ...
        <tbody>
        <slot name="row"  v-for="(item,index) in (pagesizeIn==0?info:userinfo)"  :item="item" :index="index">
          <tr>
        ...
        <slot name="row"  v-for="(item,index) in notinfo"  :item="item" :index="index">
          <tr>
            <td class="tc">{{rowIndex(index)}}</td>
            <td  v-for="column in columns">
              {{item[column.field]}}
        ...
        <slot name="row"  v-for="(item,index) in yesinfo"  :item="item" :index="index">
          <tr>
            <td class="tc">{{rowIndex(index)}}</td>
            <td  v-for="column in columns">
              {{item[column.field]}}
    ...
    <div class="table-page">
      <div class="page-info">
        <span>纪录总数:</span><span class="total">{{length}}</span><span>，共</span><span  class="total">{{page}}</span><span>页，</span>
        <span>每页显示:</span>
        ...
        <select  v-model="pagesizeIn" @change="changePagesize">
          <option value="5">5</option>
          <option value="10">10</option>
          <option value="20">20</option>
          <option value="30">30</option>
      ...
      <div class="page-pagination">
        <ul>
          <li class="first"  @click="changeFirstPage">首页</li>
          <li class="pre"  @click="changeFontPage">上一页</li>
          <!-- <li class="pre"  @click="changeFrontPage">上一页</li> -->
          <li v-for="p in page" :value="p" :class="{active:p==index}"  @click="changepage">{{p}}</li>
          <li class="next"  @click="changeNextPage">下一页</li>
          <li class="last"   @click="changeLastPage">尾页</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      setPage:function(){
        if(this.pagesizeIn!=0){
          this.page = Math.ceil(this.length/this.pagesizeIn);
        }
        
        // console.log(this.page);
      },
      changePagesize:function (e) {
        this.pagesizeIn = e.target.value;
        this.index = 1;
        this.setuserinfo();
        // this.refresh()
      },
      setuserinfo:function(){
        const userinfo = [];
        for(var i = 0 ; i < this.pagesizeIn ; i ++){
          if(this.info[i]!=undefined){
            userinfo.push(this.info[i]);
          }
        }
      ...
      changepage:function(e){
        ...
        const userinfo1 = [];
        const length1 = this.pagesizeIn;
        this.index = value;
        // console.log(this.index);
        if(value==1){
        for(var i = 0 ; i < length1 ; i ++){
            if(this.info[i]!=undefined){
              userinfo1.push(this.info[i]);
            }
          };
        }else{
          for(var i = length1 * (value-1) ; i < (length1 * value) ; i ++){
            if(this.info[i]!=undefined){
              userinfo1.push(this.info[i]);
            }
          }
          ...
          if(data==1){
            ...
            if(data1==1){
              for(var i = 0 ; i < length1 ; i ++){
                ...
                for(var i = length1 * (data1-1) ; i < (length1 * data1) ; i ++){
                  if(this.info[i]!=undefined){
                    userinfo1.push(this.info[i]);
                  }
                  this.index = data1;
